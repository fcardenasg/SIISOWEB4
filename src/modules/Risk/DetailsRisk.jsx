...
export default function DetailsRisk({ lsData = [], onDelete }) {
        ...
        <>
            <TableContainer component={Paper}>
                <Table sx={{ minWidth: 700, mb: lsData.length == 0 && 7 }} aria-label="simple table">
                    ...
                    <TableBody>
                        {stableSort(lsData, getComparator('asc', 'examenesProgramados'))
                            .slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, index) => (
                                <TableRow key={index}>
                                    <TableCell>{row.id}</TableCell>
                                    <TableCell>{row.nameGrupo}</TableCell>
                                    <TableCell>{row.clase}</TableCell>
                                    <TableCell>
                                        <Typography variant="body1">{row.usuarioRegistro}</Typography>
                                        <Typography variant="caption">{new Date(row?.fechaRegistro).toLocaleString()}</Typography>
                                    </TableCell>
                                    <TableCell>
                                        <motion.button
