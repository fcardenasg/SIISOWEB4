...
const MenuOccupationalHealth = () => {
    const navigate = useNavigate();
    const systemMenu = window.localStorage.getItem('systemMenu');
    const navigation = JSON.parse(systemMenu);
    const navigationMap = navigation.find(x => x.id === 1).children.find(x => x.id === 3).children;
                ...
                {itemsMenuButton.filter((item) => item.selected).map((item) => (
                    <Grid item xs={12} md={6} lg={3}>
                        <HoverSocialCard
                            ...
                            iconPrimary={IconClipboardHeart}
                            color={item.color}
                        />
                    </Grid>
                ))}
...
export default MenuOccupationalHealth;