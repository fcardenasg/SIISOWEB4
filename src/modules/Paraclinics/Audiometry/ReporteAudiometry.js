...
function getHeader(doc = new jsPDF()) {
    ...
    doc.text("SISTEMA DE VIGILANCIA EPIDEMIOLÓGICA PARA RUIDO", 110, 14, { align: 'center' });
    doc.text("FORMATO DE HISTORIA CLÍNICA AUDIOLÓGICA", 110, 24, { align: 'center' });
...
function generateReporteAudiometry(doc = new jsPDF(), lsDataReport = [], lsDataUser) {
    ...
    doc.text(`${lsDataReport.nameMotivo}`, 110, 18);


    doc.text('1.Registro de Ingreso', 7, 37);
    doc.text('2.Antecedentes Médicos:', 7, 57);
