...
function generateReportMedicalAdvice(doc = new jsPDF(), lsDataReport = [], lsDataUser) {
    ...
    doc.text('FECHA DE CONTRATO:', 42, 50);
    doc.text('AFP:', 42, 55);
    ...
    doc.text('ROSTER POSITION:', 120, 50);
    ...
    doc.text(`${lsDataReport.documento}`, 63, 45);
    doc.text(`${ViewFormat(lsDataReport.fechaContrato)}`, 75, 50);
    doc.text(`${lsDataReport.nameAfp}`, 50, 55);
    doc.text(`${lsDataReport.nameSubarea}`, 58, 60);
    doc.text(`${lsDataReport.nameSede}`, 52, 65);
    doc.text(`${lsDataReport.nameTurnoEmpleado}`, 54, 70);
    doc.text(`${lsDataReport.nameTelefono}`, 58, 75);

    doc.text(`${lsDataReport.nameEmpleado}`, 136, 45);
    doc.text(`${lsDataReport.nameCargo}`, 148, 50);
    doc.text(`${lsDataReport.nameDepartamento}`, 146, 55);
    doc.text(`${lsDataReport.nameGrupo}`, 132, 60);
    doc.text(`${lsDataReport.nameEps}`, 128, 65);
    doc.text(`${lsDataReport.nameDptoNacido}`, 156, 70);
    doc.text(`${lsDataReport.nameCorreo}`, 131, 75);
