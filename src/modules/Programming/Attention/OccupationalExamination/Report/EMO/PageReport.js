...
function getFirmaEmployee(doc, lsDataReport, my = 0) {
    doc.addImage(ImgWhite, "PNG", 130, doc.internal.pageSize.height - (70 - my), 50, 20);
    doc.setLineWidth(0.5);
    doc.setDrawColor(128, 128, 128);
    doc.line(130, doc.internal.pageSize.height - (48 - my), 195, doc.internal.pageSize.height - (48 - my));
    doc.setFontSize(8);
    doc.text(`${lsDataReport.nameEmpleado}`, 130, doc.internal.pageSize.height - (44 - my));
...
function generateImmunization(doc = new jsPDF(), lsDataReport) {
    const vacunas = [
        { vacuna: lsDataReport.tetanoIM, anio: lsDataReport.anioVacuna1IM, name: 'TETANO', refuerzo: '' },
        { vacuna: lsDataReport.influenzaIM, anio: lsDataReport.anioVacuna2IM, name: 'INFLUENZA', refuerzo: '' },
        { vacuna: lsDataReport.fiebreAmarillaIM, anio: lsDataReport.anioVacuna3IM, name: 'FIEBRE AMARILLA', refuerzo: '' },
        { vacuna: lsDataReport.rubeolaSarampionIM, anio: lsDataReport.anioVacuna4IM, name: 'RUBÉOLA SARAMPIÓN', refuerzo: '' },
        { vacuna: lsDataReport.covid19IM, anio: lsDataReport.anioVacuna5IM, name: 'COVID-19', refuerzo: `REFUERZOS:  ${lsDataReport.nameRefuerzoIM}` },
        { vacuna: lsDataReport.otrasIM, anio: lsDataReport.anioVacuna6IM, name: 'OTRAS', refuerzo: '' },
    ]

    if (lsDataReport.tetanoIM || lsDataReport.influenzaIM || lsDataReport.fiebreAmarillaIM ||
        lsDataReport.rubeolaSarampionIM || lsDataReport.covid19IM || lsDataReport.otrasIM) {
        ...
        doc.text(
            vacunas.filter(vacu => vacu.vacuna === true).map((vacu, index) => {
                return String(`${vacu.name}`)
            }), 7, 200, { maxWidth: 200, lineHeightFactor: 2 }
        );

        doc.text(
            vacunas.filter(vacu => vacu.vacuna === true).map((vacu, index) => {
                return String(`${vacu.anio}`)
            }), 50, 200, { maxWidth: 200, lineHeightFactor: 2 }
        );

        doc.text(
            vacunas.filter(vacu => vacu.vacuna === true).map((vacu, index) => {
                return String(`${vacu.refuerzo}`)
            }), 100, 200, { maxWidth: 200, lineHeightFactor: 2 }
        );
...
function generateParentesco(doc = new jsPDF(), lsDataReport) {
    const parentesco = [
        { name: lsDataReport.nameParentesco1ANFA, observacion: lsDataReport.parentesco1ObserANFA },
        { name: lsDataReport.nameParentesco2ANFA, observacion: lsDataReport.parentesco2ObserANFA },
        { name: lsDataReport.nameParentesco3ANFA, observacion: lsDataReport.parentesco3ObserANFA },
        { name: lsDataReport.nameParentesco4ANFA, observacion: lsDataReport.parentesco4ObserANFA },
    ]

    if (lsDataReport.nameParentesco1ANFA !== 'SIN REGISTRO' || lsDataReport.nameParentesco2ANFA !== 'SIN REGISTRO'
        || lsDataReport.nameParentesco3ANFA !== 'SIN REGISTRO' || lsDataReport.nameParentesco4ANFA !== 'SIN REGISTRO') {
        ...
        doc.text(
            parentesco.filter(paren => paren.name !== 'SIN REGISTRO').map((paren, index) => {
                return String(`${paren.name}`)
            }), 7, 130, { maxWidth: 200, lineHeightFactor: 2 }
        );

        doc.text(
            parentesco.filter(paren => paren.name !== 'SIN REGISTRO').map((paren, index) => {
                return String(`${paren.observacion}`)
            }), 50, 130, { maxWidth: 200, lineHeightFactor: 2 }
        );
...
function generateExploracionMorfologica(doc = new jsPDF(), lsDataReport) {
    ...
    doc.text(`${lsDataReport.auscultacionCardiacaEF}`, 138, 195, { align: 'right' });
    doc.text(`${lsDataReport.auscultacionRespiratoriaEF}`, 138, 200, { align: 'right' });
    ...
    /* TERCERA COLUMNA */
    doc.text(`${lsDataReport.exploracionHigadoEF}`, marXR - 2, 145, { align: 'right' });
    doc.text(`${lsDataReport.exploracionVasoEF}`, marXR - 2, 150, { align: 'right' });
    doc.text(`${lsDataReport.exploracionRinionesEF}`, marXR - 2, 155, { align: 'right' });
    doc.text(`${lsDataReport.anillosInguinalesEF}`, marXR - 2, 160, { align: 'right' });
    doc.text(`${lsDataReport.anilloUmbilicalEF}`, marXR - 2, 165, { align: 'right' });
    doc.text(`${lsDataReport.genitalesExternosEF}`, marXR - 2, 170, { align: 'right' });
    doc.text(`${lsDataReport.regionAnalEF}`, marXR - 2, 175, { align: 'right' });
    doc.text(`${lsDataReport.tactoRectalEF}`, marXR - 2, 180, { align: 'right' });
    doc.text(`${lsDataReport.tactoVaginalEF}`, marXR - 2, 185, { align: 'right' });
    doc.text(`${lsDataReport.extremidadesSuperioresEF}`, marXR - 2, 190, { align: 'right' });
    doc.text(`${lsDataReport.extremidadesInferioresEF}`, marXR - 2, 195, { align: 'right' });
    doc.text(`${lsDataReport.pulsosEF}`, marXR - 2, 200, { align: 'right' });
    doc.text(`${lsDataReport.columnaVertebralEF}`, marXR - 2, 205, { align: 'right' });
    doc.text(`${lsDataReport.articulacionesEF}`, marXR - 2, 210, { align: 'right' });

    doc.text(`${lsDataReport.especifiqueEMEFU}`, 7, 222, { maxWidth: 200, align: 'justify', lineHeightFactor: 1.5 });
}
...
function generateExamenParaclinico(doc = new jsPDF(), lsDataReport) {
    ...
    doc.text(
        parentesco.filter(exam => exam.resultado !== 'SIN REGISTRO').map((exam, index) => {
            return String(`${ViewFormat(exam.fecha)}`)
        }), 7, 137, { maxWidth: 200, lineHeightFactor: 2 }
    );
    doc.text(
        parentesco.filter(exam => exam.resultado !== 'SIN REGISTRO').map((exam, index) => {
            return String(`${exam.estudio}`)
        }), 30, 137, { maxWidth: 200, lineHeightFactor: 2 }
    );
    doc.text(
        parentesco.filter(exam => exam.resultado !== 'SIN REGISTRO').map((exam, index) => {
            return String(`${exam.resultado}`)
        }), 80, 137, { maxWidth: 200, lineHeightFactor: 2 }
    );
    doc.text(
        parentesco.filter(exam => exam.resultado !== 'SIN REGISTRO').map((exam, index) => {
            return String(`${exam.observacion}`)
        }), 110, 137, { maxWidth: 200, lineHeightFactor: 2 }
    );
}

export function generateReportConceptAptitude(doc = new jsPDF(), lsDataReport = [], lsDataUser) {
    var marXR = doc.internal.pageSize.width - 5;

    doc.text('CONCEPTO EXAMEN OCUPACIONAL', 7, 30);
    doc.text(`FECHA DE CONCEPTO:  ${ViewFormat(lsDataReport.fechaConceptoNETA)}`, doc.internal.pageSize.width - 7, 30, { align: 'right' });
    ...
    doc.setFontSize(9);
    doc.text(`${lsDataReport.recomendacionesNETA}`, 7, 87, { maxWidth: 200, align: 'justify', lineHeightFactor: 1.5 });

    getFirma(doc, lsDataUser);
}

export function generateReportDiagnosis(doc = new jsPDF(), lsDataReport = [], lsDataUser = []) {
    var marXR = doc.internal.pageSize.width - 5;
    ...
    doc.text(`RECOMENDACIONES PARA APTITUD DE ${lsDataReport.nameAtencion}`, 7, 155);
    ...
    if (lsDataReport.dxID !== undefined && lsDataReport !== []) {
        ...
        }), 7, 82, { maxWidth: 200, lineHeightFactor: 1.5 });
    }

    doc.text(`${lsDataReport.observacionID}`, 7, 103, { maxWidth: 200, align: 'justify', lineHeightFactor: 1.5 });
    doc.text(`${lsDataReport.recomendacionesID}`, 7, 163, { maxWidth: 200, align: 'justify', lineHeightFactor: 1.5 });
...
export function generateClinicHistoryOtherCompany(doc = new jsPDF(), lsDataReport = []) {
    ...
    doc.text(`FECHA:  ${ViewFormat(lsDataReport.fecha)}`, 110, 30, { align: 'center' });
    doc.text(`VALORACIÓN MÉDICA NRO:  ${lsDataReport.id}`, doc.internal.pageSize.width - 7, 30, { align: 'right' });
...
export function generateClinicHistoryDLTD(doc = new jsPDF(), resultExpoDLTD) {
    ...
    doc.text(`${resultExpoDLTD.aniosMpiDLTD + resultExpoDLTD.aniosMpiCompany} AÑOS`, 160, 97);
...
export function generatePathologicalAntecedents(doc = new jsPDF(), lsDataReport = []) {
    ...
    doc.text(`${lsDataReport.especifiqueAP}`, 7, 104, { maxWidth: 200, align: 'justify', lineHeightFactor: 1.5 });

    doc.text(`${lsDataReport.anioAT}`, 20, 133);
    doc.text(`${lsDataReport.especifiqueAT}`, 7, 142, { maxWidth: 200, align: 'justify', lineHeightFactor: 1.5 });

    doc.text(`${lsDataReport.anio1AT}`, 20, 163);
    doc.text(`${lsDataReport.especifique1AT}`, 7, 172, { maxWidth: 200, align: 'justify', lineHeightFactor: 1.5 });
...
export function generateHabitsGineco(doc = new jsPDF(), lsDataReport) {
    ...
    doc.line(5, 150, marXR, 150); /* HORI SEVEN */
    doc.line(5, 158, marXR, 158); /* HORI EIGHT */
    /* doc.line(74, 158, 74, 190); */ /* VERTI TWO */
    /* doc.line(148, 158, 148, 190); */ /* VERTI TWO */
    doc.line(5, 250, marXR, 250); /* HORI ULTIMA */
    ...
    doc.text(`${lsDataReport.cualFobiaHB}`, 40, 102, { maxWidth: 75, lineHeightFactor: 1.5 });
    doc.text(
        JSON.parse(lsDataReport.tipoFobiaHB).map((fobi, index) => {
            return String(`${fobi.label.toUpperCase()}`)
        }), 150, 97, { maxWidth: 200, lineHeightFactor: 1.5 }
    );
...
export function generateSystemReview(doc = new jsPDF(), lsDataReport) {
    ...
    doc.text(`${lsDataReport.genitoUrinarioRS}`, marXR - 2, 65, { align: 'right' });
    doc.text(`${lsDataReport.neuroRS}`, marXR - 2, 70, { align: 'right' });
    doc.text(`${lsDataReport.psiquiatricoRS}`, marXR - 2, 75, { align: 'right' });

    doc.text(`${lsDataReport.observacionRS}`, 7, 90, { maxWidth: 200, lineHeightFactor: 1.5, align: 'justify' });
