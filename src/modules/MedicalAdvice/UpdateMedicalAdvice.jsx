...
const UpdateMedicalAdvice = () => {
      ...
      if (lsServerAtencion.status === 200) {       
        ...
        if (lsServerAtencion?.data && lsServerAtencion?.data?.channel) {
          setChannelCurrent({
            ...
            fecha: lsServerAtencion?.data?.fecha,
          });
        }
        ...
        var lsResulCode = String(
          lsServerMotivo.data
            .filter((code) => code.value === lsServerAtencion.data.idMotivo)
  ...
  setTimeout(() => {
    if (lsMedicalAdvice.length !== 0) setTimeWait(true);
  }, 1500);
